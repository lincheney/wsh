...
async fn get_var(_ui: Ui, shell: Shell, lua: Lua, name: BString) -> Result<LuaValue> {
    let val = match shell.lock().await.get_var(name.as_ref())? {
...
async fn set_var(_ui: Ui, shell: Shell, lua: Lua, (name, val): (BString, LuaValue)) -> Result<()> {
    ...
    shell.lock().await.set_var(name.as_ref(), val)?;
    Ok(())
}
