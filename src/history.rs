...
async fn set_history_index(_ui: Ui, shell: Shell, _lua: Lua, val: usize) -> Result<(usize, Option<BString>)> {
    let mut shell = shell.lock().await;
    let (curhist, entry) = shell.set_curhist(val as _);
    Ok((curhist as _, entry.map(|e| e.as_entry().unwrap().text)))
}
...
async fn goto_history(ui: Ui, shell: Shell, lua: Lua, val: usize) -> Result<(usize, Option<BString>)> {
    let (curhist, text) = set_history_index(ui.clone(), shell.clone(), lua, val).await?;
...
pub async fn init_lua(ui: &Ui, shell: &Shell) -> Result<()> {
    ...
    ui.set_lua_async_fn("set_history_index", shell, set_history_index).await?;
