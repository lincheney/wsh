...
unsafe extern "C" fn handlerfunc(nam: *mut i8, argv: *mut *mut i8, options: zsh_sys::Options, func: i32) -> i32 {
...
static MODULE_FEATURES: std::sync::LazyLock<Features> = std::sync::LazyLock::new(|| {
    let name = "wash\0".to_owned();
    let nam = Box::into_raw(name.into_boxed_str()) as *mut c_char;

    let bn_list = Box::new([
        zsh_sys::builtin{
            node: zsh_sys::hashnode{ next: null_mut(), nam, flags: 0 },
